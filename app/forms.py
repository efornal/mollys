...
class PersonForm(forms.ModelForm):
    ...
    ldap_user_name = forms.CharField(
        max_length=200,
        required=False,
        label=_('ldap_user_name'))
    ldap_user_password = forms.CharField(
        min_length=settings.MIN_LENGTH_LDAP_USER_PASSWORD, 
        max_length=200,
        required=True,
        label=_('ldap_user_password'),
        widget=forms.PasswordInput())
    ldap_user_password_confirm = forms.CharField(
        min_length=8, 
        max_length=200,
        required=True,
        label=_('ldap_user_password_confirm'),
        widget=forms.PasswordInput())
    received_application = forms.BooleanField(
        required=False,
